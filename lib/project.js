...
const craftyPath = getInstalledPath.sync('crafty-cli') + '/';
...
let downloadPlugins = () => {
    ...
    let i = 1,
        progress = new pleasant();
        progress.start('Cloning');

    prefs.plugins.forEach(function(plugin){
        console.log(chalk.green('+ Cloning ' + plugin.name));
        child_process.exec('git clone ' + plugin.url + ' ' + prefs.dir + '/craft/plugins/' + plugin.name, () => {
            if (i === prefs.plugins.length) {
                progress.stop();
                setTimeout( () => pluginCleanup(), 1000 );
            }
            i++;
        });
    });
...
let pluginCleanup = () => {
    ...
    prefs.plugins.forEach(function(plugin){
        console.log(chalk.green('+ Cloning ' + plugin.name));
        child_process.exec('cd ' + prefs.dir + '/craft/plugins/' + plugin.name + ' && rm -rf .git', () => {
            if (i === prefs.plugins.length) {
                progress.stop();
